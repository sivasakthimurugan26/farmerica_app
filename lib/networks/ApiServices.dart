...
class Api_Services {
  ...
  Future<List<Product>> getMultipleCategoryProducts(List<int> categoryIds) async {
    var categories = categoryIds.join(','); // Convert the list of category IDs to a comma-separated string
    var url = "${Config.urlfor}"
        "${Config.productUrl}?category=$categories&per_page=100"
  ...
  Future createOrder({
    ...
    String? gift_from,
    String? gift_message,
    required List<CartProducts> cartProducts,
  ...
  getProduct(List<CartProducts> cartProducts) async {
    ...
    while (i < cartProducts.length) {
      Product product = await (api_services.getProductsById(cartProducts[i].product_id) as FutureOr<Product>);
      i++;
